...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        // ล็อกตาราง UserLoginRegister เพื่อป้องกันการแข่งขันข้อมูล
        mysqli_query($conn, "LOCK TABLES UserLoginRegister WRITE");

        // หาค่า user_id ล่าสุด
        $result = mysqli_query($conn, "SELECT user_id FROM UserLoginRegister ORDER BY user_id DESC LIMIT 1");
        ...
        if ($row) {
            ...
            do {
                ...
                $check_duplicate = mysqli_query($conn, "SELECT user_id FROM UserLoginRegister WHERE user_id = '$new_id'");
                $new_id_base++;
        ...
        $check_sql = "SELECT * FROM UserLoginRegister WHERE username='$username' OR email='$email'";
        ...
        $sql = "INSERT INTO UserLoginRegister (user_id, username, password, email, phone_number, first_name, last_name, created_at) 
                VALUES ('$new_id', '$username', '$password', '$email', '$phone', '$first_name', '$last_name', '$created_at')";
